codeunit 50032 "EventsSubscribers Code"
{
    ...
    local procedure C5763_OnBeforePostedWhseShptHeaderInsert(var PostedWhseShipmentHeader: Record "Posted Whse. Shipment Header"; WarehouseShipmentHeader: Record "Warehouse Shipment Header")
    var
    begin
        PostedWhseShipmentHeader."Afk Transporter Code" := WarehouseShipmentHeader."Afk Transporter Code";
        PostedWhseShipmentHeader."Afk Transporter Name" := WarehouseShipmentHeader."Afk Transporter Name";
        PostedWhseShipmentHeader."Afk Truck Code" := WarehouseShipmentHeader."Afk Truck Code";
        PostedWhseShipmentHeader.AfkCarteGrise := WarehouseShipmentHeader.AfkCarteGrise;
        PostedWhseShipmentHeader.AfkNomchauffeur := WarehouseShipmentHeader.AfkNomchauffeur;
        PostedWhseShipmentHeader.AfkPermis := WarehouseShipmentHeader.AfkPermis;
        PostedWhseShipmentHeader.AfkPrenomchauffeur := WarehouseShipmentHeader.AfkPrenomchauffeur;
    end;


    [EventSubscriber(ObjectType::Codeunit, Codeunit::"Whse.-Post Shipment (Yes/No)", 'OnAfterConfirmPost', '', true, false)]
    local procedure C5764_OnAfterConfirmPost(WhseShipmentLine: Record "Warehouse Shipment Line"; Invoice: Boolean)
    var
    ...
    local procedure C80_OnBeforeSalesShptHeaderInsert(var SalesShptHeader: Record "Sales Shipment Header"; SalesHeader: Record "Sales Header"; CommitIsSuppressed: Boolean; var IsHandled: Boolean; var TempWhseRcptHeader: Record "Warehouse Receipt Header" temporary; WhseReceive: Boolean; var TempWhseShptHeader: Record "Warehouse Shipment Header" temporary; WhseShip: Boolean; InvtPickPutaway: Boolean)
    var
    begin
        SalesShptHeader."Afk Truck Code" := TempWhseShptHeader."Afk Truck Code";
        SalesShptHeader."Afk Transporter Code" := TempWhseShptHeader."Afk Transporter Code";
        SalesShptHeader."Afk Transporter Name" := TempWhseShptHeader."Afk Transporter Name";
        SalesShptHeader.AfkCarteGrise := TempWhseShptHeader.AfkCarteGrise;
        SalesShptHeader.AfkNomchauffeur := TempWhseShptHeader.AfkNomchauffeur;
