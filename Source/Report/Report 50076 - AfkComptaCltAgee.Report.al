...
report 50076 "AfkComptaCltAgee"
{
    ...
    Caption = 'Comptabilité client âgée';
            ...
            dataitem(Customer; Customer)
            {
                ...
                dataitem(CurrencyLoop; "Integer")
                {
                    ...
                    dataitem(TempCustLedgEntryLoop; "Integer")
                    {
                        ...
                        column(AfkDescrip; CustLedgEntryEndingDate.Description)
                        {
                        }
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Âgée en date du';
                        ToolTip = 'Indique si le cumul date est calculé à partir de la date d''échéance, la date comptabilisation ou la date document.';
                    }
                    field(Agingby; AgingBy)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Agée par';
                        OptionCaption = 'Date d''échéance,Date comptabilisation,Date document';
                        ToolTip = 'Indique si le cumul date est calculé à partir de la date d''échéance, la date comptabilisation ou la date document.';
                    }
                    field(PeriodLength; PeriodLength)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Base période';
                        ToolTip = 'Spécifie la période pour laquelle les données sont affichées dans l''état. Par exemple, saisissez « 1M » pour un mois, « 30D » pour trente jours, « 3Q » pour trois trimestres ou « 5Y » pour cinq années.';
                    }
                    field(AmountsinLCY; PrintAmountInLCY)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Imprimer montant DS';
                        ToolTip = 'Indique si vous souhaitez que l''état spécifie le cumul par écriture comptable client.';
                    }
                    field(PrintDetails; PrintDetails)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Imprimer détails';
                        ToolTip = 'Indique si vous souhaitez que l''état affiche les écritures détaillées qui additionnent le solde total de chaque client.';
                    }
                    field(HeadingType; HeadingType)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Type titre';
                        OptionCaption = 'Intervalles de dates,Nombre de Jours';
                        ToolTip = 'Indique si l''en-tête de colonne pour les trois périodes doit indiquer un intervalle de date ou le nombre de jours échus.';
                    }
                    field(perCustomer; NewPagePercustomer)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Nouvelle page par client';
                        ToolTip = 'Indique si les informations client sont imprimées sur une nouvelle page si vous avez choisi plusieurs clients à inclure dans l''état, activez ce champ pour imprimer le solde de chaque client sur une page distincte.';
                    }
                    field("Skip Zero Balance Customers"; SkipZeroBalanceCustomers)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Ignorer les clients dont le solde est nul';
                        ToolTip = 'Spécifie si vous souhaitez ignorer les clients présentant un solde nul dans l’état.';
                    }
                }
            }
        }
    ...
    var
        ...
        Text000: Label 'Non échu';
        AfterTok: Label 'Après';
        BeforeTok: Label 'Avant';
        CurrencyCode: Code[10];
        Text002: Label 'jours';
        Text004: Label 'Agée par %1';
        Text005: Label 'Total pour %1';
        Text006: Label 'Agée en date du %1';
        Text007: Label 'Agée par %1';
        NumberOfCurrencies: Integer;
        Text009: Label 'Date d''écheance,Date compta,Date Document';
        Text010: Label 'La formule de la date %1 ne peut pas etre utilisée. Essayez de la reformuler. E.x. 1M+CM au lieu de CM+1M.';
        PageGroupNo: Integer;
        NextPageGroupNo: Integer;
        CustFilterCheck: Boolean;
        Text032Txt: Label '-%1', Comment = 'Durée de la période négative: %1 est la durée de la période';
        AgedAccReceivableCptnLbl: Label 'Comptabilité client âgée';
        CurrReportPageNoCptnLbl: Label 'Page';
        AllAmtinLCYCptnLbl: Label 'Tous les montants DS';
        AgedOverdueAmtCptnLbl: Label 'Montants échus âgés';
        CLEEndDateAmtLCYCptnLbl: Label 'Montant initial ';
        CLEEndDateDueDateCptnLbl: Label 'Date d''échéance';
        CLEEndDateDocNoCptnLbl: Label 'N° Document';
        CLEEndDatePstngDateCptnLbl: Label 'Date compta';
        CLEEndDateDocTypeCptnLbl: Label 'Type Document';
        OriginalAmtCptnLbl: Label 'Code devise';
        TotalLCYCptnLbl: Label 'Total (DS)';
        CurrSpecificationCptnLbl: Label 'Spécification de la devise';
        EnterDateFormulaErr: Label 'Entrez une formule de date dans le champ base période.';
        CompanyDisplayName: Text;
        TelemetryCategoryTxt: Label 'Report', Locked = true;
        AgedARReportGeneratedTxt: Label 'Etat comptabilité client âgée généré.', Locked = true;
