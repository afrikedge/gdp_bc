report 50019 "Check GDP"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Check GDP.rdlc';
    Caption = 'Check';
    ...
    dataset
    {
        ...
        dataitem(GenJnlLine;"Gen. Journal Line")
        {
            ...
            {
                column(VendLedg_PostDate;"Posting Date")
                {
                }
                column(VendLedg_DocumentNo;"Document No.")
                {
                }
                column(VendLedg_Description;Description)
                {
                }
                column(VendLedg_Currency;"Currency Code")
                {
                }
                column(VendLedg_Amount;Amount)
                {
                }
                column(VendLedg_VendName;"Vendor Name2")
                {
                }
                column(Purch_OrderNo;PurchHeader."Order No.")
                {
                }
                column(Purch_OrderDate;PurchHeader."Order Date")
                {
                }

                trigger OnAfterGetRecord()
                begin
                    PurchHeader.Get(VendorLedgEntry."Document No.");
                end;

                trigger OnPreDataItem()
                begin
                    VendorLedgEntry.SetCurrentKey("Vendor No.","Applies-to ID");
                    VendorLedgEntry.SetRange("Vendor No.",GenJnlLine."Account No.");
                    VendorLedgEntry.SetRange("Applies-to ID",GenJnlLine."Document No.");
                    if VendorLedgEntry.FindFirst then
                      Message('facture num %1', VendorLedgEntry."Document No.");
                end;
            }
    ...
    labels
    {
        Titre = 'LETTRE DE PAIEMENT PAR CHEQUE';
        Name = 'Nom';
        Adres = 'Adresse';
        Date = 'Date';
        Num = 'N°';
        ChkNum = 'Chèque N°';
        Text001 = 'Veuillez trouver ci-joint les règlements de vos factures';
        Fact = 'Facture';
        Com = 'Commande';
        Desc = 'Description';
        Mont = 'Montant';
