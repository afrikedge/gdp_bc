report 50062 "GOC Bank Transfer Order"
{
    // <changelog>
    //   <add id="FR0001" dev="EDUBUC" date="2004-08-19" area="NUMTOTEXT"  request="FR-START-40"
    //     releaseversion="FR4.00">Number-to-text conversion in check payment report (France)</add>
    //   <change id="FR0002" dev="KCOOLS" date="2005-05-17" area="NUMTOTEXT" feature="PS7850"
    //     baseversion="FR4.00" releaseversion="FR4.00.A">Code Optimization</change>
    //   <change id="FR0003" dev="KCOOLS" date="2006-03-31" area="NUMTOTEXT" feature="PS15997"
    //     baseversion="FR4.00" releaseversion="FR4.00.03">Obsolete local functionality removed</change>
    //   <change id="FR0004" dev="AUGMENTUM" date="2008-05-29" area="NUMTOTEXT" feature="PSCORS1127"
    //     baseversion="FR4.00.03" releaseversion="FR6.00">Report Transformation - localized Report Layout.</change>
    //   <change id="FR0005" dev="AUGMENTUM" date="2009-06-23" area="NUMTOTEXT" feature="NAVCORS33708"
    //     baseversion="FR6.00" releaseversion="FR6.00.01">Fix the bug about wrong total amount format.</change>
    // </changelog>
    DefaultLayout = RDLC;
    RDLCLayout = './Source/Report/Layout/GOC Bank Transfer Order.rdlc';
        ...
        dataitem(GenJnlLine; "Gen. Journal Line")
        {
            ...
            dataitem(CheckPages; "Integer")
            {
                ...
                dataitem(PrintCheck; "Integer")
                {
                    ...
                    column(VendorBankInfos1; VendorBankInfos[1])
                    {
                    }
                    column(VendorBankInfos2; VendorBankInfos[2])
                    {
                    }
                    column(VendorBankInfos3; VendorBankInfos[3])
                    {
                    }
                    column(VendorBankInfos4; VendorBankInfos[4])
                    {
                    }
                    column(VendorBankInfos5; VendorBankInfos[5])
                    {
                    }
                    column(VendorBankInfos6; VendorBankInfos[6])
                    {
                    }
                    column(VendorBankInfos7; VendorBankInfos[7])
                    {
                    }
                    column(VendorBankInfos8; VendorBankInfos[8])
                    {
                    }
                    column(VendorBankInfos9; VendorBankInfos[9])
                    {
                    }
                    column(CompanyBankInfos1; CompanyBankInfos[1])
                    {
                    }
                    column(CompanyBankInfos2; CompanyBankInfos[2])
                    {
                    }
                    column(CompanyBankInfos3; CompanyBankInfos[3])
                    {
                    }
                    column(CompanyBankInfos4; CompanyBankInfos[4])
                    {
                    }
                    column(CompanyBankInfos5; CompanyBankInfos[5])
                    {
                    }
                    column(CompanyBankInfos6; CompanyBankInfos[6])
                    {
                    }
                    column(CompanyBankInfos7; CompanyBankInfos[7])
                    {
                    }
                    column(CompanyBankInfos8; CompanyBankInfos[8])
                    {
                    }
                    column(CompanyBankInfos9; CompanyBankInfos[9])
                    {
                    }
            ...
            begin
                ...
                if not TestPrint then begin
                    ...
                    Clear(CheckToAddr);
                    //*******************************
                    Clear(VendorBankInfos);
                    Clear(CompanyBankInfos);
                    //*******************************
                    ContactText := '';
                    ...
                    case BalancingType of
                        ...
                        BalancingType::Vendor:
                            begin
                                ...
                                //**********************************
                                FillBankInfos(BankAcc2."No.", GenJnlLine."Recipient Bank Account", Vend."No.");
                                //**********************************
                                if BankAcc2."Currency Code" <> "Currency Code" then
                                    Error(Text005);
    ...
    labels
    {
        TxtFsseur = 'Fournisseur :';
        TxtLibrevilleLe = 'Libreville, Le';
        TxtFCFA = 'F CFA';
        TxtReglement = 'en règlement des factures suivantes :';
        TxtSceComptabilite = 'Service Comptabilité Fournisseur';
        PiedPage1 = 'Société d''Etat régie par la loi n° 20/2005 du 3 janvier 2006';
        PiedPage2 = 'Décret de création n° 1017/PR/MMPH du 24 août 2011';
        PiedPage3 = 'Au Capital de 1 milliard FCFA';
        PiedPage4 = 'RCCM de Libreville RG,LBV2013B14049, NIF 030280R';
        TexVir = 'Demande de virement : Ref';
        TxtBanque = 'Banque :';
        TxtEtablissement = 'Etablissement / Agence / Numéro de compte / Clé RIB :';
        TxtNomFsseur = 'Nom du fournisseur et adresse :';
        TxtRefBancaire = 'Référence bancaire du fournisseur';
        TxtNomBanque = 'Nom de la banque et adresse :';
        TxtCodeSwift = 'Code Swift (BIC) :';
        TxtCodeRIB = 'Code RIB :';
        TxtIBAN = 'IBAN :';
        TxtNumFacture = 'Numéro facture :';
        TxtMontant = 'Montant :';
        TxtDevide = 'Devise :';
    }
    ...
    var
        ...
        VendorBankInfos: array[10] of Text[50];
        CompanyBankInfos: array[10] of Text[50];
    ...
    procedure FillBankInfos(ComBankCode: Code[20]; VendorBankCode: Code[20]; VendorCode: Code[20])
    var
    ...
    begin
        ...
        if Bank2.Get(ComBankCode) then begin
            CompanyBankInfos[1] := Bank2."No.";
            CompanyBankInfos[2] := Bank2.Name;
            CompanyBankInfos[3] := Bank2.Address;
            CompanyBankInfos[4] := Bank2."Bank Account No.";
            CompanyBankInfos[5] := Bank2.IBAN;
            CompanyBankInfos[6] := Bank2."SWIFT Code";
            CompanyBankInfos[7] := Bank2."Agency Code";
            CompanyBankInfos[8] := Format(Bank2."RIB Key");
            CompanyBankInfos[9] := Bank2."Bank Branch No."; //Code etablissement
        end;

        if VendorBankAcc.Get(VendorCode, VendorBankCode) then begin
            VendorBankInfos[1] := VendorBankAcc.Code;
            VendorBankInfos[2] := VendorBankAcc.Name;
            VendorBankInfos[3] := VendorBankAcc.Address;
            VendorBankInfos[4] := VendorBankAcc."Bank Account No.";
            VendorBankInfos[5] := VendorBankAcc.IBAN;
            VendorBankInfos[6] := VendorBankAcc."SWIFT Code";
            VendorBankInfos[7] := VendorBankAcc."Agency Code";
            VendorBankInfos[8] := Format(VendorBankAcc."RIB Key");
            VendorBankInfos[9] := VendorBankAcc."Bank Branch No." //Code etablissement
        end;
        //*************************************************************************************************JN00001
    end;
}
